...
public class MissingRepatingNumberFinder {
 ...
 class MissingRepeatingNumberSolution {
    ...
    public int[] repeatedNumber(final int[] inputArray) {
        ...
        sumOfN = ((long)arraySize) * ( ((long)arraySize + 1) / 2);
        sumSquareOfN =  (((long)arraySize)*(((long)arraySize + 1)*((long)2*arraySize + 1)))/6;

        System.out.println("Sum of N : " + sumOfN);
        System.out.println("Sum Square of N : " + sumSquareOfN);

        for(int loopIndex = 0; loopIndex < arraySize ; loopIndex++){
            actualSum += (long) inputArray[loopIndex];
            actualSumSquare +=  (long) Math.pow((long)inputArray[loopIndex] , 2);
        }

        System.out.println("actualSum of N : " + actualSum);
        System.out.println("actualSumSquare of N : " + actualSumSquare);

         missingNumber = (int) ( ( (sumOfN - actualSum)
                         + ((actualSumSquare - sumSquareOfN) / (actualSum - sumOfN )) ) / 2 );

        repeatedNumber = (int) (actualSum - sumOfN) + missingNumber;
