...
public class DiagonalTraverseTester2 {
	/**
	 * @param args
	 */
	public static void main(String[] args) {
		 ...
		 */
		 DiagonalArrayTraverser diagonalTraverser = new DiagonalArrayTraverser();
...
class DiagonalArrayTraverser {
	public ArrayList<ArrayList<Integer>> findDiagonalOrder(ArrayList<ArrayList<Integer>> matrix) {
		...
		ArrayList<ArrayList<Integer>> diagonalOrder = new ArrayList<ArrayList<Integer>>();
		ArrayList<Integer> innerDiagonalOrderList = new ArrayList<>();
		int outputIndex = 0;
		int row = 0;
		int column = 0;
		int direction = 0;
		...
		while (row <= (inputRowSize - 1) && column <= (inputColumnSize - 1)) {
			...
			if (row == 0 && column == 0) {
				...
				innerDiagonalOrderList.add(matrix.get(row).get(column++));
				diagonalOrder.add(innerDiagonalOrderList);
				innerDiagonalOrderList = new ArrayList<>();
				direction = 1;
				continue;
			}
			// Downwards traversal
			if (direction == 1) {
				if (row < (inputColumnSize - 1) && column > 0) {
					//diagonalOrder[outputIndex++] = matrix[row++][column--];
					innerDiagonalOrderList.add(matrix.get(row++).get(column--));
				} else if (row < (inputColumnSize - 1) && column == 0) {
					...
					innerDiagonalOrderList = new ArrayList<>();
					direction = 0;
				} else if (row == (inputColumnSize - 1) && column <= (inputColumnSize - 1)) {
					//diagonalOrder[outputIndex++] = matrix[row][column++];
					innerDiagonalOrderList.add(matrix.get(row).get(column++));
					diagonalOrder.add(innerDiagonalOrderList);
					innerDiagonalOrderList = new ArrayList<>();
			...
			// Upwards traversal
			if (direction == 0) {
				if (row > 0 && column < (inputColumnSize - 1)) {
					//diagonalOrder[outputIndex++] = matrix[row--][column++];
					innerDiagonalOrderList.add(matrix.get(row--).get(column++));
					...
					direction = 1;
						...
						|| row < (inputColumnSize - 1) && column == (inputColumnSize - 1)) {
				 ...
				 * diagonalOrder[outputIndex++] = matrix[row++][column]; direction = 1; }
				 */
				else if (row == (inputColumnSize - 1) && column == (inputColumnSize - 1)) {
					//diagonalOrder[outputIndex++] = matrix[row][column];
					innerDiagonalOrderList.add(matrix.get(row).get(column));
